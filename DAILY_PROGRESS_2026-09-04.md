...
3. 图片存档类条目（相机命名照片）是占位文本，需要人工整理或 OCR 后补充
4. URL 网页抓取录入联测（`POST /api/knowledge/add-url` 仍待测）
5. 摄像头作背景联测；VAD 灵敏度调参；句级气泡重叠合并显示
6. 用户复检：App 登录→对话问校情（现应有真实数据支撑），观察检索命中率
7. 功能点.md 项目二（数智化成长平台）未开始
