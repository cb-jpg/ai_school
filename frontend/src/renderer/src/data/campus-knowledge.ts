...
/**
 * 从后端 API 获取专题数据
 * 如果 API 请求失败，则回退到本地静态数据
 */
export async function fetchTopicFromAPI(topicId: CampusTopicId): Promise<CampusTopic> {
  ...
  try {
    ...
    switch (topicId) {
      ...
      case 'history':
        ...
        break;
      case 'achievements':
        endpoint = '/api/topics/achievements';
        break;
      case 'role-models':
        endpoint = '/api/topics/students';
        break;
      default:
    ...
    // 转换 API 数据为前端格式
    return convertAPIToTopic(topicId, data);
  } catch (error) {
    console.warn(`Failed to fetch topic ${topicId} from API, using local data:`, error);
    // 回退到本地数据
    return campusTopicMap[topicId];
  }
}

/**
 * 将 API 响应转换为前端 Topic 格式
 */
    ...
    case 'role-models': {
      const sections: CampusKnowledgeSection[] = (data.students || []).map((student: any) => {
        ...
        const methodDescriptions = Object.entries(student.methods || {})
          .filter(([_, methods]: [string, any]) => methods && methods.length > 0)
...
/**
 * 获取所有专题数据（优先从 API，失败则使用本地数据）
 */
export async function fetchAllTopics(useAPI = true): Promise<CampusTopic[]> {
  if (!useAPI) {
    return campusTopics;
  }
  ...
  try {
    ...
    console.warn('Failed to fetch topics from API, using local data:', error);
    return campusTopics;
  }
}
